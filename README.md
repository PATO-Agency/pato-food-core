...
TypeScript monorepo for a configurable food-business landing page and structured Sanity editing. The local reference app uses fixtures and requires no customer account or secret.
...
`npm run check` runs lint, type checks, tests, secret-pattern checks and workspace builds. `npm run format` formats implementation files. CI validates Windows and Linux without publishing. Source packages export TypeScript and are transpiled by consuming apps. This repository is not a package release or an automatic client provisioning system.
...
No cart, payments, CRM, chatbot, WhatsApp API, automated bookings or multi-tenant dashboard are included. The implemented baseline is accepted for the internal pilot; ownership, external accounts and every production approval remain explicit open decisions.
